        ...
        if (data) {
          ...
          if (category === 'clubes') {
            ...
            baseData = baseData.filter(p => {
              ...
              const model = (p.model_type || '').toLowerCase();
              return cat.includes('retro') || cat.includes('retrô') || model === 'retro' || model === 'retrô';
            });
          } else if (category === 'artes-custom' || category === 'personalizados') {
            baseData = baseData.filter(p => {
              const cat = (p.category || '').toLowerCase();
              return cat.includes('custom') || cat.includes('personalizado');
            });
          }
